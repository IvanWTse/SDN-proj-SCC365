...
"""
Ryu Template Controller for NAT Router coursework
...
class Router(RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            ...
            sys.exit(1)
        self.interface_table = InterfaceTable()
    ...
    # HELPER FUNCTIONS
    # ----------------
    # The functions below do not directly support the NAT router,
    # however may be useful in the development of other controllers.

    def __print_dict(self, dict_to_print, message="", debug=True):
        """
        Pretty Print a Dictionary

        Prints a dictionary in JSON format via the controller logger. The
        `debug` parameter specifies if the output should go to the debug log
        output. If false, the output will go though the info log output.
        If message is set, it will be printed above the outputted dict though
        the same log output.
        """
...
"""
Interfaces Table (extends Table)

At first, this will contain an empty table as this data is not to be loaded from
an external JSON. However, during runtime this table should be populated as
datapaths join. This contains helper functions for accessing the data and
altering the data.
"""
...
class InterfaceTable(Table):
    ...
    def add_interface(self,  dpid: str, port: int, hw: str, ip: str):
        """
        Add Interface

        Add an interface to the interfaces table.
        """
        self._table.setdefault(dpid, [])
        (self._table[dpid]).append({
            "port": port,
            "hw": hw,
            "ip": ip
        })
